...
interface ModuleTwoClientProps {
  meta: CourseMeta
  lessons: Lesson[]
  quiz: QuizQuestion[]
}

export default function ModuleTwoClient({ meta, lessons, quiz }: ModuleTwoClientProps) {
      ...
      <div className="space-y-5 pb-8">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
            {lang === 'es' ? `${quiz.length} preguntas • Necesitas ${meta.passThreshold * 100}% para aprobar` : `${quiz.length} questions • Need ${meta.passThreshold * 100}% to pass`}
          </p>
        </div>
        <QuizPlayer questions={quiz} passThreshold={meta.passThreshold} onComplete={handleQuizComplete} />
      </div>
    )
  }
