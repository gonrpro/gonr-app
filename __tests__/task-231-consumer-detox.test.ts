...
const FORBIDDEN_PROMPT_TERMS: Array<{ label: string; re: RegExp }> = [
  ...
  { label: 'StreePRO', re: /streepro/i },
  { label: 'Mulsolite', re: /mulsolite/i },
  { label: 'PYRATEX', re: /pyratex/i },
  { label: 'Formula 209 (incl. "General Formula No. 209")', re: /(?:general\s+)?formula\s*(?:no\.?\s*)?209/i },
...
describe('TASK-231 — consumer output guard', () => {
  ...
  const leakCases: Array<[string, string]> = [
    ...
    ['streepro', 'Use StreePRO per shop practice.'],
    ['mulsolite', 'Flush with Mulsolite first.'],
    ['pyratex', 'PYRATEX lifts the resin.'],
    ['formula-209', 'General Formula No. 209 works here.'],
